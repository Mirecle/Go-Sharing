...
def compress_image(infile, outfile='', mb=3072, step=1, quality=40):
    ...
    if o_size <= mb:
        print('init',o_size,mb)
        return infile
    outfile = get_outfile(infile, outfile)
    while o_size > mb:
        print('while',o_size,mb)
...
def checksize(image):
    ...
    if M%128!=0:
        ...
        m=M
    if N%128!=0: 
        print('width not suit,remain%d'%(N%128))
        n=N-N%128
    else:
        n=N
...
if __name__ == "__main__":
    ...
    if operation == 'checksize':
        print(checksize(insert.readImage(oimage)))
    elif operation =='resize':
        print(resize_image(oimage,sys.argv[3],int(sys.argv[4]), int(sys.argv[5])))
    elif operation == 'compress':
        print(compress_image(oimage,sys.argv[3],int(sys.argv[4]),int(sys.argv[5]),int(sys.argv[6])))