    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        for image, _ in train_data:
            ...
            message = torch.Tensor(np.random.choice([0, 1], (image.shape[0], hidden_config.message_length))).to(
                    device)
            losses, _ = model.train_on_batch([image, message],only_decode)
        ...
        for image, _ in val_data:
            ...
            losses, (encoded_images, noised_images, decoded_messages) = model.validate_on_batch([image, message],only_decode)
            for name, loss in losses.items():
